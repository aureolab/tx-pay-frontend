...
interface PaymentLinkDetailDialogProps {
  item: PaymentLink | null;
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function PaymentLinkDetailDialog({
  item,
  open,
  onOpenChange,
}: PaymentLinkDetailDialogProps) {
  ...
  const handleDownloadQr = async (format: 'png' | 'svg' | 'pdf') => {
    ...
    try {
      let response;
      if (format === 'png') {
        response = await paymentLinksApi.downloadQrPng(item._id);
      } else if (format === 'svg') {
        response = await paymentLinksApi.downloadQrSvg(item._id);
      } else {
        ...
        <DialogHeader className="px-6 pt-5 pb-0">
          <DialogTitle className="flex items-center gap-3 text-lg">
            ...
            <span className="text-zinc-900 dark:text-zinc-50">
              {t('dialogs.paymentLinkDetail.title', 'Detalle de Link de Pago')}
            </span>
          </DialogTitle>
            ...
            <div className="border-t border-zinc-100 dark:border-zinc-800/80 pt-4">
              ...
              <div className="grid grid-cols-2 gap-4 mt-2">
                <div>
                  ...
                  <Badge variant="outline" className="mt-1 bg-blue-500/5 text-blue-700 dark:text-blue-300 border-blue-500/20">
                ...
                <div>
                  ...
                  <Badge variant="outline" className="mt-1 bg-indigo-500/5 text-indigo-700 dark:text-indigo-300 border-indigo-500/20">
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                {item.max_uses && (
                  <div>
                    <Label className="text-zinc-500 dark:text-zinc-400 text-xs">Maximo de Usos</Label>
                    <p className="font-medium text-zinc-900 dark:text-white mt-0.5">{item.max_uses}</p>
                  </div>
                )}
                {item.expires_at && (
                  <div>
                    <Label className="text-zinc-500 dark:text-zinc-400 text-xs">Expira</Label>
                    <p className="font-medium text-zinc-900 dark:text-white mt-0.5">
