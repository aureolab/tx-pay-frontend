...
import { adminUsersApi, partnerUsersApi } from '../../api/client';
...
export function ResetPasswordDialog({
  ...
  const handleReset = async () => {
    ...
    try {
      let response;
      if (userType === 'admin') {
        response = await adminUsersApi.resetPassword(userId);
      } else {
        response = await partnerUsersApi.resetPassword(userId);
      }
