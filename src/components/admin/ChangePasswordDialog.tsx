...
export function ChangePasswordDialog({ open, onOpenChange, onSuccess, userId, userName }: ChangePasswordDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (newPassword.length < 8) {
      setError(t('dialogs.changePassword.minLengthError'));
      return;
    }
    setLoading(true);
    setError('');
    try {
      ...
      setError(err.response?.data?.message || t('dialogs.changePassword.changeError'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="max-w-sm bg-white dark:bg-zinc-900 border-zinc-200/80 dark:border-zinc-800/80 shadow-xl shadow-amber-900/5 dark:shadow-amber-900/20 p-0 gap-0 overflow-hidden">
        ...
        <DialogHeader className="px-6 pt-5 pb-0">
          ...
          <DialogDescription className="mt-1.5 pl-12 text-zinc-500 dark:text-zinc-400">
            {t('dialogs.changePassword.description', { name: userName })}
          </DialogDescription>
        </DialogHeader>
